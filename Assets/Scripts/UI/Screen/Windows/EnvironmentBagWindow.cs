...
public class EnvironmentBagWindow : BagWindow
{
    ...
    private void OnMove(EnvironmentBag curEnvironmentBag)
    {
        ...
        foreach (var state in stateStateSliders)
        {
            ...
            {
                stateStateSliders[EnvironmentStateEnum.Height].SetActive(true);
            }
            else
            {
                stateStateSliders[EnvironmentStateEnum.Height].SetActive(false);
            }
    ...
    private void OnEnvironmentChangeState(RefreshEnvironmentStateArgs args)
    {
        ...
        {
            if (stateStateSliders.ContainsKey(args.state) && GameManager.Instance.CurEnvironmentBag.EnvironmentStateDict.ContainsKey(args.state))
            {
                //float resultValue = 0;
