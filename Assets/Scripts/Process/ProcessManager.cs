...
{
    private static List<ProcessConfig> processConfigList;

    static ProcessManager()
    {
        processConfigList = ExcelReader.ReadProcess("ProcessConfig");
    }

    public static string GetProcessOutcomeID(List<Card> processedCards, Dictionary<TemperatureType, int> temperatureRecord)
    {
    ...
    /// <returns></returns>
    private static List<ProcessConfig> FindMatchedProcess(List<Card> processedCards, Dictionary<TemperatureType, int> temperatureRecord)
    {
    ...
    /// <returns></returns>
    private static ProcessConfig PickFromMatchedProcess(List<ProcessConfig> matchedConfigs)
    {
        // 按照优先级降序排序
    ...
    /// <param name="processedCards"></param>
    /// <returns></returns>
    private static bool IsFoodPropertyRequirementMet(List<FoodPropertyRequirement> requirements, List<Card> processedCards)
    {
    ...
    /// <param name="processedCards"></param>
    /// <returns></returns>
    private static bool IsCardRequirementMet(List<CardRequirement> requirements, List<Card> processedCards)
    {
        foreach (var re in requirements)
        {
            // 统计卡牌数量
    ...
    /// <param name="temperatureRecord"></param>
    /// <returns></returns>
    private static bool IsTempertureRequirementMet(ProcessConfig config, Dictionary<TemperatureType, int> temperatureRecord)
    {
        int roundCount = 0;
